...
\displaystyle\frac{1}{\tilde{\mathcal{N}}\left(S\right)} \exp \left\lbrace -\displaystyle\frac{1}{2} (\boldsymbol{x}-\boldsymbol{\mu})^\top S^{-1} (\boldsymbol{x}-\boldsymbol{\mu})\right\rbrace ,
...
\hat{S_k} &= \displaystyle\frac{1}{N_k} \displaystyle\sum_{n=1}^N \gamma_{nk} (\boldsymbol{x}_n - \hat{\boldsymbol{\mu}}_k) (\boldsymbol{x}_n - \hat{\boldsymbol{\mu}}_k)^\top,
\end{aligned}
\right\rbrace 
\enspace\mathrm{where}\enspace
...
これは、分散 $W$ の $D$ 変量正規分布に従う $\nu$ 個の変数 $\boldsymbol{x}_n$ の直積 $\boldsymbol{x}_n\boldsymbol{x}_n^\top$ の和の分布である。即ち、標本分散の分布である。
...
- \displaystyle\frac{1}{2} \underset{\boldsymbol{\mu},S}{\mathbf{E}}\!\left[\,D \log 2\pi + \log \left|S_k\right| + (\boldsymbol{x}_n - \boldsymbol{\mu}_k)^\top S_k^{-1} (\boldsymbol{x}_n - \boldsymbol{\mu}_k)\,\right]. \qquad(6.37)$$
...
$$\underset{\boldsymbol{\mu},S}{\mathbf{E}}\!\left[\,(\boldsymbol{x}_n - \boldsymbol{\mu}_k)^\top S_k^{-1} (\boldsymbol{x}_n - \boldsymbol{\mu}_k)\,\right] =
\nu_k (\boldsymbol{x}_n - \boldsymbol{m}_k)^\top W_k (\boldsymbol{x}_n - \boldsymbol{m}_k) + \displaystyle\frac{D}{\sigma_k}. \qquad(6.39)$$
...
\left\lbrace D \log 2\pi + \left|S_k\right| + (\boldsymbol{x}_n-\hat{\boldsymbol{\mu}}_k)^\top S_k^{-1} (\boldsymbol{x}_n-\hat{\boldsymbol{\mu}}_k) - 2 \log w_k\right\rbrace . \qquad(6.41)$$
...
$$\hat{W}_k^{-1} =
W_k^{-1}+\displaystyle\sum_{n=1}^N\gamma_{nk}\boldsymbol{x}_n\boldsymbol{x}_n^\top+\sigma_k\boldsymbol{m}_k\boldsymbol{m}_k^\top-\hat{\sigma}_k\hat{\boldsymbol{m}}_k\hat{\boldsymbol{m}}_k^\top. \qquad(6.45)$$

式 6.45の導出では、分散 $W$ の**コレスキー分解**により下三角行列 $T$ が存在して、式 6.46が成立する性質を利用した。

$$\boldsymbol{x}^\top W \boldsymbol{x} =
(\boldsymbol{x}^\top T) (T^\top \boldsymbol{x}) =
(T^\top \boldsymbol{x})^\top (T^\top \boldsymbol{x}) =
\mathrm{tr}\left((T^\top \boldsymbol{x}) (T^\top \boldsymbol{x})^\top\right) =
\mathrm{tr}\left(T^\top (\boldsymbol{x} \boldsymbol{x}^\top) T\right) =
\mathrm{tr}\left((\boldsymbol{x} \boldsymbol{x}^\top) W\right). \qquad(6.46)$$
